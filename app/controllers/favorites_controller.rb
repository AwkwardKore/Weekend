class FavoritesController < ApplicationController
  def create
    ...
    if @favorite.save
       redirect_to(:back, alert: "Se agregó a favoritos: #{Place.find(params[:id]).name}")
    else
  ...
  def destroy
    if current_user.favorite_places.find(params[:favorite_id]).destroy
      redirect_to(place_path(params[:id]), alert: "Se quitó de favoritos: #{Place.find(params[:id]).name}")
    else
      @place = Place.find(params[:id])
      flash.now[:error] = "There was an error."
      render 'places/show'
